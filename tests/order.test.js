...
describe("Order API Tests (CMD Input)", function () {
  ...
  it("should fetch all orders", async function () {
    ...
    expect(response.status).to.equal(200);
    expect(response.body.message).to.equal("Order Listed Sucessfully");
  });
  ...
  it("should update an order", async function () {
    ...
    const response = await makeRequest(options, postData);
    expect(response.status).to.equal(200);
    expect(response.body.message).to.equal("updated the product data");
  });
  ...
  it.skip("should delete an order", async function () {
    ...
    const response = await makeRequest(options, postData);
    expect(response.status).to.equal(200);
    expect(response.body.message).to.equal("delete the product sucessfully");
  });
});
